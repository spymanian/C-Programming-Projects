...
int main(int argc, char *argv[])
{
	...
	if (!bv) {
		...
		#pragma omp parallel for schedule(guided,200) shared(wl_size, num_hf, wl, bv_size) private(i,j, hash) 
	...
	#pragma omp parallel for schedule(guided,200) shared(wl_size, num_hf, wl, bv_size) private(j, hash)
